...
The `WorkflowExecutor` interface SHALL define a single `execute(graph: CompiledDAG, nodeRunner: NodeRunner, ctx: Context): Promise<ExecutionResult>` method that decouples orchestration strategy from DAG structure. The `ctx` parameter provides the execution context directly — it SHALL NOT be smuggled through the graph object.

#### Scenario: Interface contract
- **WHEN** a class implements `WorkflowExecutor`
- **THEN** it SHALL accept a `CompiledDAG`, `NodeRunner`, and `Context` and return a `Promise<ExecutionResult>`
...
- **WHEN** `loadNode("../../../../etc/something.ts", workflowDir)` is called
- **THEN** `loadNode()` SHALL throw an Error with message containing "resolves outside workflow directory"
- **AND** the file SHALL NOT be loaded
