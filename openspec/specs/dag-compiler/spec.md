...
### Requirement: Workflow spec validation
The `compile()` function SHALL validate the workflow spec before compilation, checking for required fields.

#### Scenario: Missing name
- **WHEN** a workflow spec has no `name` field (or a non-string name)
- **THEN** `compile()` SHALL throw an Error with message containing `"name"`

#### Scenario: Missing nodes
- **WHEN** a workflow spec has no `nodes` field or an empty `nodes` object
- **THEN** `compile()` SHALL throw an Error with message containing `"nodes"`

#### Scenario: Missing edges
- **WHEN** a workflow spec has no `edges` field or a non-array `edges`
- **THEN** `compile()` SHALL throw an Error with message containing `"edges"`
