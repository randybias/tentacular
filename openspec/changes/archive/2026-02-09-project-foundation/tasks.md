...
- [x] 1.2 Create `cmd/pipedreamer/main.go` with cobra root command and global flags (namespace, registry, output, verbose)
- [x] 1.3 Register all command stubs: init, validate, dev, test, build, deploy, status, cluster check, visualize
- [x] 1.4 Implement `pipedreamer init <name>` — scaffold workflow.yaml, nodes/hello.ts, .secrets.yaml.example, tests/fixtures/hello.json
- [x] 1.5 Add kebab-case name validation to init command
- [x] 1.6 Create `pkg/` directory structure: cli/, builder/, k8s/, spec/

## 2. Deno Engine Setup

- [x] 2.1 Create `engine/deno.json` with strict TS config, import map (std/yaml, std/path, std/flags, std/assert, pipedreamer), lint/fmt settings
...
- [x] 3.1 Create directory structure: cmd/pipedreamer/, pkg/cli/, pkg/builder/, pkg/k8s/, pkg/spec/, engine/compiler/, engine/executor/, engine/context/, engine/testing/, examples/, tests/
- [x] 3.2 Create `.gitignore` covering binaries, vendor, .deno, secrets, IDE files

## 4. Verification

- [x] 4.1 Verify `go build ./cmd/pipedreamer/` compiles successfully
- [x] 4.2 Verify `pipedreamer --help` shows all commands
- [x] 4.3 Verify `pipedreamer init test-workflow` creates correct scaffold
