...
The `engine/mod.ts` SHALL export the `Context`, `Logger`, and `NodeFunction` types for use by workflow node authors.

#### Scenario: Module exports
- **WHEN** `engine/mod.ts` is imported
- **THEN** it SHALL export at minimum: `Context`, `Logger`, `NodeFunction`, `WorkflowSpec`, `ExecutionResult`
