...
- Implement `tntc init <name>` to scaffold new workflow directories
...
**Choice:** `cmd/tntc/` for Go entrypoint, `pkg/` for Go packages, `engine/` for Deno code.
...
**Rationale:** Ensures `tntc --help` shows the full command surface from day one. Stubs return "not yet implemented" errors. Each subsequent change fills in its command.
...
- **Stub commands may confuse users** → Running `tntc dev` before Change 05 is implemented returns an error. Acceptable for incremental development.
