...
- [x] 1.1 Update "Build Phase" section (line 275): replace monolithic build steps with engine-only flow — generate engine-only Dockerfile, copy engine, docker build, default tag `pipedreamer-engine:latest`, save to `.pipedreamer/base-image.txt`
...
- [x] 2.3 Update Build > "Image Tag" (line 47): change default tag to `pipedreamer-engine:latest`
...
- [x] 2.6 Add new "Fast Iteration" section after Deploy: explain edit code → `pipedreamer deploy` → ConfigMap updated + rollout restart → done (no Docker build needed)
