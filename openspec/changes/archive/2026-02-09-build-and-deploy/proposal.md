...
Pipedreamer v2 workflows must be packaged as container images and deployed to Kubernetes. The Go CLI currently has stub implementations for `build`, `deploy`, and `status` commands. This change replaces those stubs with working implementations that generate Dockerfiles, build container images, generate K8s manifests, apply them via client-go, and report deployment status. This is the sixth change in the dependency chain, building on the project foundation (01), workflow spec (02), DAG engine (03), context/secrets (04), and dev command (05).

## What Changes

- **`pipedreamer build [dir]`** — generates a Dockerfile using `denoland/deno:distroless` base, copies the engine and workflow files into the build context, and invokes `docker build` to produce a tagged container image
- **`pipedreamer deploy [dir]`** — generates K8s manifests (Deployment with gVisor RuntimeClass, ClusterIP Service), applies them to the target namespace via client-go, with secrets mounted as K8s Secret volumes (not environment variables)
- **`pipedreamer status <name>`** — queries the K8s API for deployment health, reporting replica counts, availability, and readiness in text or JSON format
...
- **K8s manifest generation** (`pkg/builder/k8s.go`) — produces Deployment (gVisor RuntimeClass, resource limits, secret volume mounts) and Service (ClusterIP) manifests with pipedreamer labels
...
- **CLI**: `pipedreamer build`, `pipedreamer deploy`, and `pipedreamer status` commands transition from stubs/initial implementations to fully specified behavior
