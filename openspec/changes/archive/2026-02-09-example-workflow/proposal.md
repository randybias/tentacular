...
Tentacular has all the core components (engine, compiler, executor, context, testing framework, CLI commands) but no reference workflow that demonstrates how they work together. Without a working end-to-end example, developers have no concrete starting point, and the project cannot validate that `tntc validate`, `tntc dev`, and `tntc test` actually work with a real workflow. The github-digest example serves as both documentation and integration smoke test.
...
- **Test fixtures** (`tests/fixtures/fetch-repos.json`, `tests/fixtures/summarize.json`) — JSON files with `{ "input": ..., "expected": ... }` format for node-level testing via `tntc test`
...
- **Dependencies**: No new dependencies; all nodes use the existing `tentacular` module import and Deno std library
- **Verification**: `tntc validate examples/github-digest` must pass, `tntc test examples/github-digest` must pass with fixtures, `tntc dev examples/github-digest` must start the dev server
