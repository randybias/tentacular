...
- Default image tag `pipedreamer-engine:latest` when `--tag` not specified
- Persist the built image tag to `.pipedreamer/base-image.txt` in project root for Phase 2 consumption
...
### 4. Default image tag is `pipedreamer-engine:latest`

**Decision:** When `--tag` is not specified, default to `pipedreamer-engine:latest`.
...
**Alternatives considered:** (a) Use CLI version constant (`pipedreamer-engine:<cli-version>`) — adds coupling between image tag and CLI versioning. (b) Require `--tag` explicitly — too much friction for the common case. Team-lead confirmed `pipedreamer-engine:latest` as the right default.

### 5. Tag persisted to `.pipedreamer/base-image.txt` in project root

**Decision:** After successful build (and optional push), write the full image tag to `.pipedreamer/base-image.txt` relative to the current working directory (project root). Create `.pipedreamer/` if it doesn't exist.
...
**[Risk] `.pipedreamer/` directory may not be in `.gitignore`** — Contains local build state that shouldn't be committed.
